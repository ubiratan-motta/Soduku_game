...
def Preenchendo_Quadrantes(tabuleiro_data, x2, y2):
    ...
    while quadrante_preenchido == True:
        ...
        if tabuleiro_data[y][x] == 'n' and numero not in linha_sorteada and numero not in coluna_sorteada and numero \
                not in quadrante:
...
def Checando_Numero_Digitado(tabuleiro_data, jogo_data, click_position_x, click_position_y, numero):
    x = click_position_x
    y = click_position_y
    if x >= 0 and x <= 8 and y >= 0 and y <= 8 and tabuleiro_data[y][x] == numero and jogo_data[y][x] == 'n' \
            and numero != 0:
        jogo_data[y][x] = numero
        numero = 0
    if x >= 0 and x <= 8 and y >= 0 and y <= 8 and tabuleiro_data[y][x] == numero and jogo_data[y][x] == numero \
            and numero != 0:
        pass
    if x >= 0 and x <= 8 and y >= 0 and y <= 8 and tabuleiro_data[y][x] != numero and jogo_data[y][x] == 'n' \
            and numero != 0:
        jogo_data[y][x] = 'X'
        numero = 0
    if x >= 0 and x <= 8 and y >= 0 and y <= 8 and tabuleiro_data[y][x] == numero and jogo_data[y][x] == 'X' \
            and numero != 0:
        jogo_data[y][x] = numero
        numero = 0
...
def Click_Botao_Restart(mouse_position_x, mouse_position_y, click_last_status, click, tabuleiro_preenchido, escondendo_numeros,
                        tabuleiro_data, jogo_data):
    x = mouse_position_x
    y = mouse_position_y
...
while True:
    ...
    tabuleiro_preenchido, escondendo_numeros, tabuleiro_data, jogo_data = Click_Botao_Restart(mouse_position_x,
                                                                                              mouse_position_y, click_last_status,
                                                                                              click, tabuleiro_preenchido,
                                                                                              escondendo_numeros, tabuleiro_data,
                                                                                              jogo_data)
